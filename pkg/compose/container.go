...
func positionChild(parent Widget, child Widget, position func() (x, y unit.Dp)) {
...
func (c *container) Layout(gtx layout.Context, n node.Node, w WidgetPainter) (dims layout.Dimensions) {
	child := paint.Group(gtx.Ops, func() {
		defer c.PhaseRecorder.Trigger(layout.PhaseBeforeSize)
		defer func() {
			c.PhaseRecorder.RecordSize(gtx.Metric.PxToDp(dims.Size.X), gtx.Metric.PxToDp(dims.Size.Y))
			c.PhaseRecorder.Trigger(layout.PhaseDidSize)
		}()

		dims = c.Detector.LayoutChild(
			gtx,
