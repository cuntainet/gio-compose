...
var _ gesture.HandlersSetter = &inputWidget{}
...
func (w inputWidget) New(ctx context.Context) internal.Widget {
	return &inputWidget{
		Element: node.Element{
			Name: w.Name,
		},
...
func (w *inputWidget) Update(ctx context.Context, modifiers ...modifier.Modifier[any]) bool {
	// reset events
	w.Events = textinput.Events{}
...
func (w *inputWidget) Layout(gtx layout.Context) layout.Dimensions {
	w.container.BindFocusedChecker(&w.editor)

	return w.container.Layout(gtx, w, WidgetPainterFunc(func(gtx layout.Context) layout.Dimensions {
		for _, event := range w.editor.Events() {
			if _, ok := event.(widget.ChangeEvent); ok {
				w.textEmitted = w.editor.Text()
				w.Events.TriggerChange(w.textEmitted)
			}
